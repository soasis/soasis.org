...
There's an additional `using fallible = std::true_type`, because infrastructure in the Standard Library (or a different library) should recognize that this is a fallible allocator and thusly can return an invalid result, until the infallible `.allocate()` functions of old.

Note that this is templated because, internally, an algorithm or similar could have the need to allocate things that do not exactly match for the allocator that was given. In this case, the library writer using the `fallocator` would need to [use the `allocator_traits::rebind_alloc` functionality to get a new `fallocator`](https://en.cppreference.com/w/cpp/memory/allocator_traits) so they can allocate objects of that type.


## Using the Fallible Allocator

It would be used like a normal allocator through the standard allocator traits:

```cpp
using alloc_traits = std::allocator_traits<decltype(alloc)>;
auto maybe_allocate = ::allocate(alloc, num_elements);
if (!maybe_allocate) {
	// panic!!
}
// good to go
...
void stable_sort(const _BidIt _First, const _BidIt _Last, _Pr _Pred, _Al _Alloc) {
    ...
    using _Alloc_traits_type = _STD allocator_traits<decltype(_Get_unwrapped(_Alloc)>;
    ...
    // allocate buffer
    const auto _Maybe_temp_buf = _Alloc_traits_type::allocate(_Alloc, _Capacity);
    if (_Maybe_tmp_buf) {
...
And there it is! A stable sort, but one that allows the end-user to exert control over the final version of the implementation. This, of course, applies to other algorithms (`std::inplace_merge`, `std::stable_merge`) as well as some common implementation strategies for parallel algorithms. The `fallocator` demonstrated here and its usage follow the existing allocator API very strictly, but honestly there's a lot of room for improvement in its design and its choice of API. That will be left for another day, because there is a **lot** to talk about! But, there is something I've done with this API here that I snuck in against the advice of my infinite intellectual and engineering betters...

It's not the most important thing. In fact, beyond this point, there be dragons 🐉! It's alright if you stop here. In fact, it's probably encouraged!
...
It returns an `std::optional`. Remember, the `non_null` is just a template that does literally nothing, except maybe help some folks who do shenanigans with static analyzers. This means that, ultimately, the API is working with `std::optional<T*>`. This can enable a particularly fun, non-standard use case that has no real grounding to exist! That is,

You can return `nullptr` from the allocator.



## ... What. W-Why?!

Okay, so. Hear me out here, before you slam dunk on me (you can dunk on me later, but your slam dunk should be filled with the righteousness of rejecting my folly). There are cases where the value of the `nullptr` --- in the case where it has an actual representation value of `0` --- actually ends up _pointing to valid memory_. This typically happens in "unprotected" development or low-level embedded development: whether it's mapped registers or other shenanigans, sometimes the literal address `0` is, indeed, not nothing, but something.

<a id="fnref-Alexander-Payne"></a>Normally, the fix to this is easy: the compiler can make the `nullptr` representation something like `0x55555555`. But sometimes, the compiler doesn't exactly do that, as [Alexander Payne <sup>1</sup>](#fn-Alexander-Payne) explains...
...
But, how effective is this actually? As [Nika Layzell pointed out<sup>2</sup>](#fn-Nika-Layzell) if you try out some code that exposes a `nullptr` -- even if you don't check explicitly for `if (p == nullptr)`, it's easy to see the [compiler makes assumptions about the `nullptr`-ness](https://godbolt.org/z/rc4eYj):
...
You can see from this code that it never checks for `i == 5`, because the double-dereference in `my_thing` invalidates that check inside of the for loop So, if we never do the `5` branch, then there's only `2` actual branches in the combined code. The first is being less than `100`, in which case there is always a value. The second is if it is not, in which case the return value is always `0`. The compiler -- after inlining that method -- proves that because we dereference both the `std::optional` _and_ the `int*`, so at no point can `i` be `5` because our code dereferences the pointer stored _inside_ the `std::optional`.

So the compiler removes it.

But! There's a way to cheat, using what Alexander told us:
...
<a id="fnref-Mara-Bos"></a>Ah! And for a moment, we can celebrate! And yet... it's actually still not correct. See, even if we never compare against `nullptr`, and even if we squirrel the value of the `nullptr` type in a different translation unit OR go so far as to bury it in a Linker Script, we can still run afoul of the compiler. As [Mara Bos<sup>3</sup>](#fn-Mara-Bos) explained during our conversation:
...
And it doesn't stop there: [Trap Pointer Representations](https://cigix.me/c17#6.3.2.3.p5) are a thing in both C and C++, too, and any integral value converted into a pointer could trigger a hard trap. This means that even just returning `the_real_nullptr` could take a live trap round and load it into the register and destroy you at runtime. But, beyond just the theoretical implications, there's also the practical one, that Mara also pointed out:
...
And she's right. What kind of allocator _really_ needs to return the actual `0` pointer value? Is it worth it to return a `std::optional<non_null<T*>>` when just returning a `T*` would do? There might be an embedded use case out there, but an embedded use case that needs it in an allocator...? That's pretty suspicious! So, maybe we can make `allocate` just return a `T*` again. But... you know. That's something we'll have to figure out, another day! For now, we'll stick to better (parallel) algorithms.
...
<a id="fn-Alexander-Payne"></a>1 — [Alexander Payne](https://myrrlyn.net/) is a software developer. He [works on `bitvec`](https://crates.io/crates/bitvec) and other amazing things that help make the world go 'round. [⤴](#fnref-Alexander-Payne)
