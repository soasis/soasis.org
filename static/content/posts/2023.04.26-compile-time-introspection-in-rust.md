...
This article will spend a perhaps obscene amount of time reasoning, designing, critiquing, comparing, pontificating, and yearning for this or that feature, functionality, idiom, or capability. Whatever assertions are contained below, it is critical to recognize that we have one and only one goal in mind for this project. That is, we want — in the [primary serialization crate featured across the entire Rust ecosystem, `serde`](https://docs.rs/serde/latest/serde/) — David Tolnay (or any one of the 159 contributors)'s code to be able to handle this:
